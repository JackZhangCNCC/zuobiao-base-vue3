        ...
        <div class="advanced-search-form">
          <a-form layout="vertical" :model="tempFormState" ref="formRef">
            ...
            <div class="advanced-search-buttons">
              <a-button @click="handleCancelAdvanced">取消</a-button>
              <a-space>
                <a-button @click="handleAdvancedReset" :disabled="loading">重置</a-button>
                <a-button type="primary" :loading="loading" :disabled="loading" @click="handleAdvancedSearch">查询</a-button>
              </a-space>
            </div>
          </a-form>
        </div>
...
const handleValueChange = () => {
  ...
  Object.keys(formState).forEach(key => {
    if (key === 'dateRange' && Array.isArray(formState[key])) {
      // 使用时间戳检查日期是否有效，避免响应式对象问题
      const value = formState[key];
      if (value.length === 2) {
        const startValid = value[0] && !isNaN(value[0].getTime ? value[0].getTime() : new Date(value[0]).getTime());
        const endValid = value[1] && !isNaN(value[1].getTime ? value[1].getTime() : new Date(value[1]).getTime());
        
        if (!startValid || !endValid) {
          console.warn('检测到无效日期，已重置');
          formState[key] = [];
        }
      }
    }
...
const formatTagValue = (tag) => {
  ...
  if (tag.type === "select") {
    ...
    // 日期格式化
    return tag.value ? tag.value.format(tag.format || "YYYY-MM-DD") : "";
  } else if (tag.type === "dateRange") {
    // 日期范围格式化
    if (Array.isArray(tag.value) && tag.value.length === 2) {
      const format = tag.format || "YYYY-MM-DD";
      return `${tag.value[0].format(format)} 至 ${tag.value[1].format(format)}`;
    }
    return "";
  }
  // 默认直接返回值
...
const handleSearch = () => {
  ...
  Object.keys(formState).forEach((key) => {
    ...
    if (
      ...
      // 对日期范围进行特殊处理
      if (Array.isArray(value) && key === 'dateRange') {
        // 使用时间戳来判断日期有效性，避免instanceof Date的问题
        const hasValidDates = value.length === 2 && 
          value[0] !== null && value[1] !== null &&
          !isNaN(value[0] && value[0].getTime ? value[0].getTime() : new Date(value[0]).getTime()) &&
          !isNaN(value[1] && value[1].getTime ? value[1].getTime() : new Date(value[1]).getTime());
        
        if (hasValidDates) {
          params[key] = value;
        } else {
          console.warn('搜索参数中包含无效日期，已忽略:', key, value);
        }
      } else {
        params[key] = value;
      }
...
const handleAdvancedSearch = () => {
  ...
  Object.keys(tempFormState).forEach(key => {
    if (key === 'dateRange' && Array.isArray(tempFormState[key])) {
      // 使用时间戳判断日期有效性
      const value = tempFormState[key];
      const hasValidDates = value.length === 2 && 
        value[0] !== null && value[1] !== null &&
        !isNaN(value[0] && value[0].getTime ? value[0].getTime() : new Date(value[0]).getTime()) &&
        !isNaN(value[1] && value[1].getTime ? value[1].getTime() : new Date(value[1]).getTime());
...
const handleReset = () => {
  ...
  Object.keys(formState).forEach((key) => {
    if (key === 'dateRange') {
      formState[key] = []; // 对日期范围使用空数组而不是undefined
    } else {
